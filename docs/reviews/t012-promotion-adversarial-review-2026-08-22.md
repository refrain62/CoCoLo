...
実装範囲のレビューでは Critical 0件 / High 0件。Node.js 24を前提に、ローカル検証とCIのPostgreSQL検証を分けて判定する。
...
- 実装コミット: `143a328`、`a2c2e38`、`ce22360`、`50f7907`、`e005d73`、`3e5313c`、`56b5812`、`cb7ca22`

## 敵対的確認

| 観点 | 確認結果 |
| --- | --- |
| テナント越境 | tenantIdをリクエストから受け取らず、active membershipを同一transaction内で`FOR UPDATE`確認。tenant行・対象member行・PromotionRunをRLS下でロックし、member更新は`tenantId + id`で実行する。tenant A/BのPostgreSQL統合テストを追加。 |
...
| 冪等性・競合 | tenant行ロックで同一tenantの並行年度処理を直列化。同一年度のcompletedは保存済み結果を返し、同一keyのhash変更・年度変更・key変更は409。Web executeは毎回ランダムkeyを生成する。 |
...
- `pnpm test`: 成功
- `pnpm test:unit`: 成功（Vitest 8件、API 14件、contracts/domain含む）
- `pnpm build`: 成功
- `pnpm typecheck`: 成功
...
- `pnpm --filter @cocolo/api test:integration`: ローカルはDATABASE_URL未設定かつDocker API利用不可のため未実行。PR CIのPostgreSQL実行結果を最終判定に使用する。
...
実Supabase staging接続、deploy adapter、staging専用ユーザー、外部サービス接続はT-009〜T-011の運用課題であり、T-012のコード上のCritical / Highではない。CIの実PostgreSQL統合テストが成功するまでT-012を最終完了扱いにしない。
