...
| `[!]` | ADMIN-TEAM-001-ACCEPTANCE | ログイン後回帰修正と全体お知らせ公開境界はPR #233で`develop`統合済み。stagingでSupabaseの`system_admin` claim付与、migration適用、実DBのRLS（公開publishedのみ・tenant越境不可）、実ブラウザの`/admin`・`/team`・`/dashboard`受入を確認する（実装履歴は[完了履歴](resume-task-history.md)） |
