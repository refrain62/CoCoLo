...
実装再開の基準となる`develop`は`7a0b023`（PR #112〜#121反映後）です。実装baselineは`0903dd6`（PR #104、#105を含む）で、添付APIは`ced1e71`、添付Webは`e4d7af0`、注文・集金APIは`e708e96`、注文Webは`750c08e`、送迎・logout・LINE接続操作は`8c96baf`まで、Betterleaks秘密情報検査は`7a0b023`まで、LINE通知outboxの接続先・接続世代検証は`cb5b63f`まで反映済みです。完了済み実装とdocs-only PRの履歴更新は`develop`へ反映済みです。
