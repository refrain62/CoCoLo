...
| 状態 | ID | 次に確認すること |
| --- | --- | --- |
| `[!]` | T014-ROOT / T014-SCAN / T014-PR | `main`のowner-only bootstrapはPR #51、UUIDv7検査のowner-only拡張はPR #247・#249・#250、trusted PRのowner-only extension適用境界はPR #259・#260、protected path削除契約はPR #264・#267、owner-only登録PR検証ゲートはbootstrap #271と実装PR #272で`develop`へ統合済み。scanner root境界はPR #302、最終head登録はPR #308、scanner実装はPR #309で統合済み。developのruleset `develop-trust-and-ci-gate`（required `quality` / `trusted-validation`、bypassなし）も設定済み。残りはscanner実行、rename・削除経路のGitHub API外部実行確認、同一SHAのrelease証跡を同一rootで確認する |
