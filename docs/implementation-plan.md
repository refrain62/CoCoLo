...
管理画面は単一画面の条件分岐を増やさず、システム管理の`/admin`と、選択中チームを管理する`/team`のルートおよび機能メニューへ分割します。
利用者向けの予定・締め切り画面は`/dashboard`で提供します。
...
7. **公開LPと管理UI**：未認証ルートのLP、認証状態による画面分離、`/admin`・`/team`・`/dashboard`ルート、メニュー、共通UI、デザイントークン、role別とfeature別の受入。
