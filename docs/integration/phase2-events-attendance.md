...
同じイベント、同じ回答者、同じ部員の組み合わせにはunique制約を置き、repositoryはイベント行をロックしたatomic upsertで同じ行を更新します。

管理者が回答者を指定しない修正では、既存回答があればその回答者を保持します。複数guardianの回答は部員ごとの最新回答だけを集計し、部員数を二重計上しません。
