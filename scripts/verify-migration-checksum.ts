...
export function migrationPaths(root = repositoryRoot) {
...
export function formatMigrationManifest(
...
export function parseMigrationManifest(content: string): MigrationChecksum[] {
  assert.ok(
    !content.startsWith('\uFEFF'),
    'checksum manifestはBOMなしにしてください。',
  );
  assert.ok(
    !content.includes('\r'),
    'checksum manifestはLF改行にしてください。',
  );
  assert.ok(
    content.endsWith('\n'),
  ...
  for (const [index, line] of lines.entries()) {
    ...
    assert.ok(
      groups.path,
      `checksum manifest ${index + 1}行目のパスが空です。`,
    );
    assert.ok(
      groups.sha256,
    ...
    assert.ok(
      !paths.has(groups.path),
      `${groups.path}: manifestで重複しています。`,
    );
    paths.add(groups.path);
    entries.push({ path: groups.path, sha256: groups.sha256 });
  }
  return entries;
}
...
export function verifyMigrationManifest(
  ...
  assert.equal(
    formatMigrationManifest(expected),
    formatMigrationManifest(actual),
...
export async function readMigrationChecksums(
  ...
  const { migrationsDirectory } = migrationPaths(root);
