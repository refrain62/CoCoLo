...
test('owner-only extensionは登録済みheadと変更集合を完全一致させる', () => {
  const changedFiles = [
    trustedScannerFiles[0],
    trustedScannerFiles[1],
  ] as const;
  const headHashes = Object.fromEntries(
    changedFiles.map((file) => [file, 'a'.repeat(64)]),
  );
  const extension = {
    schema: 1 as const,
  ...
  assert.throws(() =>
    assertBootstrapTrustedChanges(extension.head_sha, headHashes, extension, [
      ...changedFiles,
