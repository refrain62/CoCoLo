...
serialTest(
  '同一tenantで別sourceがIdempotency-Keyを再利用しても500ではなく409になる',
  async () => {
...
serialTest(
  '業務transactionのenqueueからworker claim・送信・sent確定まで実DBで完了する',
  async () => {
...
serialTest(
  'retry・unknown・lease切れは同じprovider retry keyで重複送信を抑止する',
  async () => {
...
serialTest(
  'unknown確定は古いtokenまたは期限切れleaseでは状態を変更しない',
  async () => {
    const notificationId = await publishViaProductionApi({
...
serialTest(
  'enqueueはmembership変更とFOR UPDATEで直列化し、停止後の通知登録を拒否する',
  async () => {
...
serialTest(
  'worker接続は専用role・RLS非bypassでclaim関数だけを利用する',
  async () => {
