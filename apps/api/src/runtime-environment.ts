...
function assertUrl(name: string, value: string) {
  ...
  if (url.protocol !== 'https:' && url.hostname !== '127.0.0.1')
    throw new Error(`${name}はHTTPSまたはlocal loopback URLが必要です`);
}
...
export function readRuntimeEnvironment(
  ...
  if (r2Bucket !== allowedBuckets[appEnv])
    throw new Error('R2_BUCKETが環境の許可値と一致しません');

  if (appEnv === 'local' && publicAppUrl !== 'http://localhost:5173')
    throw new Error('PUBLIC_APP_URLがlocal環境の許可値と一致しません');
  ...
  const allowlist = environment.PUBLIC_APP_URL_ALLOWLIST?.split(',')
    ...
    .filter(Boolean);
  if (appEnv !== 'local') {
    if (!allowedUrl) throw new Error('SUPABASE_ALLOWED_URLが必要です');
    if (!allowedJwksUrl) throw new Error('SUPABASE_ALLOWED_JWKS_URLが必要です');
    if (!allowlist?.length)
      throw new Error('PUBLIC_APP_URL_ALLOWLISTが必要です');
  }
  if (allowedUrl && allowedUrl.replace(/\/$/, '') !== supabaseUrl)
    throw new Error('SUPABASE_URLが許可された環境値と一致しません');
  if (allowedJwksUrl && allowedJwksUrl !== supabaseJwksUrl)
    throw new Error('SUPABASE_JWKS_URLが許可された環境値と一致しません');
  if (allowlist && !allowlist.includes(publicAppUrl))
    throw new Error('PUBLIC_APP_URLが許可リストに含まれていません');
