-- 切断・再接続でLINEの送信先が変わった後に、古い世代のoutboxをclaimしない。
...
COMMENT ON FUNCTION app_claim_line_delivery_outbox(integer, integer) IS '現在のLINE接続世代と一致する通知だけを短時間transactionでclaimする';

-- claim後、外部送信直前にも接続世代を再検証し、切断・再接続済みなら送信しない。
CREATE FUNCTION app_validate_line_delivery_claim(
  p_tenant_id uuid,
...
COMMENT ON FUNCTION app_validate_line_delivery_claim(uuid, uuid, uuid) IS '外部送信直前にLINE接続世代とattempt tokenを再検証し、古い通知をunknownへ確定する';
